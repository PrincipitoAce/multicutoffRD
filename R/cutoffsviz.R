...
#'
#' @param data A data set with the following values: Original Cutoffs, New Cutoffs, & Difference
...
cutoffsviz = function(data, color){
  ...
  # Plotting
  toPlot |>
    ggplot(mapping = aes(x = Group, y = Value, color = `Cutoff Type`)) +
    geom_point() +
    scale_color_manual(values = c("New Cutoffs" = color[1], "Original Cutoffs" = color[2]))
}
